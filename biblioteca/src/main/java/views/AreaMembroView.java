...
public class AreaMembroView extends javax.swing.JPanel {
    ...
    private static final java.util.logging.Logger logger = java.util.logging.Logger
            .getLogger(AreaMembroView.class.getName());
    ...
    void setMembro(Membro m) {
        this.membroLogado = m;
        lblNomeMembro.setText("Bem-vindo(a), " + m.getNome());
        boolean suspenso = Membro.listarMembrosSuspensos().stream().anyMatch(s -> s.getCpf().equals(m.getCpf()));
        lblStatusSuspensao.setText(suspenso ? "Status: Suspenso" : "Status: Ativo");
        carregarAcervo();
    ...
    private void carregarAcervo() {
        ...
        for (Livro l : livroController.listarLivros()) {
            modelo.addRow(new Object[] {
                    ...
                    l.getAnoPublicacao(), l.getDisponiveis()
            });
        }
    }

    private void carregarMeusEmprestimos() {
        DefaultTableModel modelo = (DefaultTableModel) tabelaMeusEmprestimos.getModel();
        modelo.setRowCount(0);
        if (membroLogado == null)
            return;
        for (Emprestimo e : emprestimoController.listarEmprestimos()) {
            if (e.getMembro().getCpf().equals(membroLogado.getCpf())) {
                modelo.addRow(new Object[] {
                        e.getLivro().getIsbn(),
